...
import { saveNote, deleteNote, setBookShelf } from '../../actions'
...
export function NotesClient({
  ...
  function startEditNote(note: typeof notes[0]) {
    ...
    // On mobile the editor sits below the feed, so an Edit tap would otherwise
  ...
  return (
    <div className="space-y-6 px-5 md:px-8">
      ...
      {/* Personal notes are the whole point of this page. Chapter tracking
          lives on the home screen and the chapter completion flow, so this page
          only reads progress back — it never writes it. */}
          ...
          {notes.length === 0 ? (
            <Card className="p-8 text-center border-dashed">
              ...
              <p className="text-sm text-[var(--text-secondary)]">
                No notes saved yet. Use the note editor to write your first one.
              </p>
            </Card>
          ) : (
      ...
      {/* Chapter history. Read-only on purpose: the two flows that log chapters
          keep free text out of chapter_completions.reflection_text, which RLS
          exposes to the whole group. */}
      <div className="space-y-3">
        ...
        {completions.length === 0 ? (
          <p className="text-sm text-[var(--text-secondary)]">
            No chapters logged yet. Chapters you complete will show up here.
          </p>
        ) : (
          <div className="space-y-3">
